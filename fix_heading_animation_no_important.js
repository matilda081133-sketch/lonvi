...
console.log("Removing !important from gradient-text-animated to allow keyframe animation execution...");

// Replace the animation rule block with a clean, !important-free version for the gradient
const cleanAnimBlock = `
<style>
...
// Wait! If keyframe animations cannot override !important properties in some engines, let's declare background-position explicitly in the keyframe as !important?
// No! Keyframes with !important are actually IGNORED in CSS! (That is also in the spec: !important in keyframes is ignored).
// So the correct solution is to remove !important from the background-position and background properties in the main style declaration!
// Wait! If we remove !important from 'background' and 'background-size' in the stylesheet, let's see:
const finalPolishStylesClean = `
...
<style>
    ...
    body .gradient-text-animated,
    .gradient-text-animated {
        background: linear-gradient(90deg, #FFFFFF 0%, #7E8694 25%, #F0F2F5 50%, #4C5363 75%, #FFFFFF 100%); /* NO !important here */
        background-size: 300% auto; /* NO !important here */
        -webkit-background-clip: text; /* NO !important here */
...
html = html + '\n' + finalPolishStylesClean;

fs.writeFileSync(file, html, 'utf8');
console.log("tilda_shop_page.html updated without !important on heading gradient.");
